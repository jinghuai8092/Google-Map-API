...
function UTCtoLocationTime(utc_offset){
    ...
    var locationTimestamp=UTC_Timestamp+utc_offset*60;
    var LD=new Date(locationTimestamp);
    return `${LD.getFullYear()}-${LD.getMonth()+1<10? '0'+(LD.getmonth()+1):LD.getMonth()+1}-${LD.getDate()} ${date.getHours()}:${date.getMinutes()}:${date.getSeconds()}:`
}
...
function showPosition(Position) {
    ...
    var marker = new google.maps.Marker({ Position: latlon, map: map, title: "You are here!" })
      ...
      searchBox.addListener('places_changed', function () {
        ...
        locationAndTime.innerHTML=name+'\n';
